        ...
        <div class="container">
	      ...
	      <div class="col-md-9">
           ...
           <div class=" col-md-2">
              ...
              //while($row=mysqli_fetch_array($query)){
              ?>
                <input type="text" class="form-control pull-right" id="date" name="remark" placeholder="Remark" tabindex="2"  required>
              <?php ?>
							</div><!-- /.input group -->
						</div><!-- /.form group -->
					 </div>
...
<?php
		...
		$query=mysqli_query($con,"select * from temp_trans natural join product natural join category where branch_id='$branch'")or die(mysqli_error());
			$grand=0;
		while($row=mysqli_fetch_array($query)){
